...
namespace ACT.EDT
{
    public class ActMinMaxSliderControl
    {
        ...
        private static ActMinMaxSliderControl.MinMaxSliderState s_MinMaxSliderState;
        ...
        internal static void DoMinMaxSlider(Rect position, int id, ref float value, ref float size, float visualStart, float visualEnd, float startLimit, float endLimit, GUIStyle slider, GUIStyle thumb, bool horiz)
        {
            ...
            ActMinMaxSliderControl.MinMaxSliderState minMaxSliderState = ActMinMaxSliderControl.s_MinMaxSliderState;
            ...
            switch ((int)current.GetTypeForControl(id))
            {
                case 0:
                    ...
                    {
                        minMaxSliderState = (ActMinMaxSliderControl.s_MinMaxSliderState = new ActMinMaxSliderControl.MinMaxSliderState());
                    }
                    ...
                    if (slider != GUIStyle.none)
                    {
                        if (size != 0f & flag)
                        {
                            ...
                            minMaxSliderState.whereWeDrag = 0;
                            GUI.changed = true;
                            ActMinMaxSliderControl.s_NextScrollStepTime = DateTime.Now.AddMilliseconds((double)ActMinMaxSliderControl.kFirstScrollWait);
                ...
                case 7:
                    ...
                    if (DateTime.Now >= ActMinMaxSliderControl.s_NextScrollStepTime)
                    {
                        float num13 = (!horiz) ? current.mousePosition.y : current.mousePosition.x;
                        float num14 = (!horiz) ? rect.y : rect.x;
                        ...
                        value = Mathf.Clamp(value, num3, num4 - size);
                        ActMinMaxSliderControl.s_NextScrollStepTime = DateTime.Now.AddMilliseconds((double)ActMinMaxSliderControl.kScrollWait);
                    }
                    return;
                default:
                    return;
        ...
        {
            int controlID = GUIUtility.GetControlID(ActMinMaxSliderControl.repeatButtonHash, focusType, position);
            EventType typeForControl = Event.current.GetTypeForControl(controlID);
            //if (typeForControl == null)
            //{
        ...
        public static void MinMaxScroller(Rect position, int id, ref float value, ref float size, float visualStart, float visualEnd, float startLimit, float endLimit, GUIStyle slider, GUIStyle thumb, GUIStyle leftButton, GUIStyle rightButton, bool horiz)
        {
            ...
            ActMinMaxSliderControl.MinMaxSlider(position2, ref value, ref size, num2, num3, num2, num3, slider, thumb, horiz);
            bool flag = false;
            if (Event.current.type == EventType.MouseUp)
            {
                flag = true;
            }
            if (ActMinMaxSliderControl.ScrollerRepeatButton(id, rect, leftButton))
            {
                value -= num * ((visualStart >= visualEnd) ? -1f : 1f);
            }
            if (ActMinMaxSliderControl.ScrollerRepeatButton(id, rect2, rightButton))
            {
                value += num * ((visualStart >= visualEnd) ? -1f : 1f);
            }
            if (flag && Event.current.type == EventType.Used)
            {
                ActMinMaxSliderControl.scrollControlID = 0;
            }
        ...
        {
            ActMinMaxSliderControl.DoMinMaxSlider(position, GUIUtility.GetControlID(ActMinMaxSliderControl.s_MinMaxSliderHash, FocusType.Passive), ref value, ref size, visualStart, visualEnd, startLimit, endLimit, slider, thumb, horiz);
        }
        private static bool ScrollerRepeatButton(int scrollerID, Rect rect, GUIStyle style)
        {
            bool result = false;
            if (ActMinMaxSliderControl.DoRepeatButton(rect, GUIContent.none, style, FocusType.Passive))
            {
                bool arg_22_0 = ActMinMaxSliderControl.scrollControlID != scrollerID;
                ActMinMaxSliderControl.scrollControlID = scrollerID;
                if (arg_22_0)
                {
                    result = true;
                    ActMinMaxSliderControl.nextScrollStepTime = Time.realtimeSinceStartup + 0.001f * (float)ActMinMaxSliderControl.firstScrollWait;
                }
                else
                {
                    if (Time.realtimeSinceStartup >= ActMinMaxSliderControl.nextScrollStepTime)
                    {
                        result = true;
                        ActMinMaxSliderControl.nextScrollStepTime = Time.realtimeSinceStartup + 0.001f * (float)ActMinMaxSliderControl.scrollWait;
                    }
                }
            }
            return result;
