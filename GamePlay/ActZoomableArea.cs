...
namespace ACT.EDT
{
    ...
    public class ActZoomableArea : MonoBehaviour
    {
        ...
        private ActZoomableArea.Styles styles;
        ...
        public ActZoomableArea()
        {
            ...
            this.styles = new ActZoomableArea.Styles();
        }
        public void BeginViewGUI(bool handleUserInteraction)
        {
            ...
            {
                int controlID = GUIUtility.GetControlID(ActZoomableArea.zoomableAreaHash, 0, drawArea);
                switch (Event.current.GetTypeForControl(controlID))
                {
                    case EventType.MouseDown:
                        if (drawArea.Contains(Event.current.mousePosition))
                        {
                            ...
                            if (this.IsZoomEvent() || this.IsPanEvent())
                            {
                                ...
                                ActZoomableArea.m_MouseDownPosition = this.mousePositionInDrawing;
                                Event.current.Use();
                            }
                        }
                        break;
                    case EventType.MouseUp:
                        if (GUIUtility.hotControl == controlID)
                        {
                            ...
                            ActZoomableArea.m_MouseDownPosition = new Vector2(-1000000f, -1000000f);
                        }
                        break;
                    case EventType.MouseDrag:
                        if (GUIUtility.hotControl == controlID)
                        {
                            if (this.IsZoomEvent())
                            {
                                this.Zoom(ActZoomableArea.m_MouseDownPosition, false);
                                Event.current.Use();
                            }
                            else
                            {
            ...
            GUILayout.EndArea();
            this.horizontalScrollbarID = GUIUtility.GetControlID(ActMinMaxSliderControl.s_MinMaxSliderHash, FocusType.Passive);
