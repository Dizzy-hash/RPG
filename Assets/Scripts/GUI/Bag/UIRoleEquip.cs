...
public class UIRoleEquip : GTWindow
{
    ...
    {
        if (mAvatar == null && mAvatar.RootObj != null)
            return;
        ESpin.Get(mAvatar.RootObj).OnSpin(delta, 2);
    }
    ...
    private void ShowModelView()
    {
        ...
        GameObject model = mRender.AddModel(mAvatar.RootObj);
    ...
    private void ShowPropertys()
    {
        ...
        XCharacter role = RoleModule.Instance.GetCurPlayer();
        Dictionary<EAttr, int> propertys = AttrModule.GetPropertys(role);
