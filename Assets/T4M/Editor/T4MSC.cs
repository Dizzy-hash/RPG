...
public class T4MSC : EditorWindow {
    ...
    [MenuItem("Window/Batch T4M Source Codes %t")]
	...
	void Obj2T4M()
	{
        ...
        CurrentSelect.gameObject.GET<MeshRenderer>();
    ...
    public void TerrianBatch()
    {
        for (int i = 1; i <= 400; i++)
        {
            CurrentSelect = AssetDatabase.LoadAssetAtPath<GameObject>(string.Format("Assets/Test/Map/1/Map_{0}.prefab", 201000 + i)).transform;
            FinalExpName = string.Format("Map_{0}", 201000 + i);
            ConvertUTerrain();
        }
    }
